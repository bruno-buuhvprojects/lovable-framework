...
1. **Define routes** and call `registerRoutes(routes)` so the framework can match URLs and run `getData` when needed.
...
- **Route data context**: initial data from SSR (`window.__PRELOADED_DATA__`) and data fetched on client when you navigate to a route that has `getData`.
